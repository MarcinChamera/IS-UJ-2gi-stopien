...
void MPIDataProcessor::singleExecution() {
	...
	if (rank == 0 && numOfProcesses > 1) {
		int counter = 0;
		for (int i = columnsInCurrentProcess - 1; i < columnsInCurrentProcess; i++) {
			for (int j = 0; j < dataSize; j++) {
	...
	for (int col = margin; col < columnStop; col++) {
		for (int row = margin; row < dataSize - margin; row++) {
			...
			// std::cout << function->calc(dataPortionBuffer) << std::endl;
	...
	// jako proces 0: 
		...
		int dataFromOtherProcessesSize = (dataSize - 2 * margin) * (dataSize - columnsInCurrentProcess - margin);
		double *calculatedDataFromOtherProcesses = new double[dataFromOtherProcessesSize];
		MPI_Status status;
		int sizeToReceive;
		double *p = calculatedDataFromOtherProcesses;
		for (int i = 1; i < numOfProcesses; i++) {
			sizeToReceive = i != numOfProcesses - 1 ? (dataSize - 2 * margin) * columnsNotLastProcess : (dataSize - 2 * margin) * (columnsLastProcess - 1);
			// MPI_Recv((&calculatedDataFromOtherProcesses) [(i - 1) * columnsNotLastProcess], sizeToReceive, MPI_DOUBLE, i, i , MPI_COMM_WORLD, &status);
		...
		for (int j = columnsNotLastProcess; j < dataSize - margin; j++)
			for (int i = margin; i < dataSize - margin; i++) {
				...
				// nextData[i][j] = calculatedDataFromOtherProcesses[(i - margin) * dataSize + (j - columnsNotLastProcess)];
		}
		...
		// 		}
		// 		MPI_Recv(calculatedDataFromOtherProcess, sizeToReceive, MPI_DOUBLE, processNumber, 0, MPI_COMM_WORLD, &status);
		// 	}
			// // zapisz obliczone komorki z pozostalych procesow
			// for (int i = 0; i < dataSize; i++) {
